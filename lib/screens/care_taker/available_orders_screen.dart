...
class _AvailableOrdersScreenState extends State<AvailableOrdersScreen>
  ...
  static const _accent = Color(0xFF25A98F);
  ...
  // Tracks which order id is currently being accepted (per-card spinner)
  ...
  Future<void> _load({bool silent = false}) async {
    try {
      ...
      // GET /caretaker/orders/:category — backend filters CONFIRMED + caretaker_id IS NULL
      final res = await http
          .get(Uri.parse("${Api.baseUrl}/caretaker/orders/${widget.category}"));
  ...
  // ── QUICK ACCEPT — one tap from the list, no need to open details ──
  Future<void> _acceptOrder(Map order) async {
    final orderId = order["id"];
    if (orderId == null || _accepting.contains(orderId)) return;
    ...
    try {
      final res = await http
          .post(
            Uri.parse("${Api.baseUrl}/caretaker/accept"),
            headers: {"Content-Type": "application/json"},
            body: jsonEncode({
              "order_id": orderId,
              "caretaker_id": widget.userId,
            }),
          )
      ...
      if (data["success"] == true) {
        _snack("Booking accepted! It's now yours ✓");
        if (mounted) {
          setState(() {
            _orders.removeWhere((o) => o["id"] == orderId);
            _accepting.remove(orderId);
          });
        }
      } else {
        _snack(data["message"] ?? "Could not accept — try another booking", error: true);
        if (mounted) setState(() => _accepting.remove(orderId));
        // Someone else may have grabbed it — refresh the list quietly
        _load(silent: true);
      }
    } catch (_) {
  ...
  Widget _card(Map order) {
    ...
    final isAccepting = _accepting.contains(orderId);
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                if (code.isNotEmpty)
                  Container(
                    ...
                    decoration:
                        BoxDecoration(gradient: _gradient, borderRadius: BorderRadius.circular(8)),
                ...
                Container(
                  ...
                  decoration:
                      BoxDecoration(gradient: _gradient, borderRadius: BorderRadius.circular(20)),
                  child: Text("₹$total",
                      style: const TextStyle(
              ...
              if (category.isNotEmpty)
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: _primary.withOpacity(0.08), borderRadius: BorderRadius.circular(6)),
                  child: Text(category,
                      style: const TextStyle(
              ...
              // ── BIG QUICK-ACCEPT BUTTON ──
              _acceptButton(order, isAccepting),

              const SizedBox(height: 8),
              Center(
                child: Text("or tap the card to view full details",
  ...
  Widget _acceptButton(Map order, bool isAccepting) => SizedBox(
              ...
              decoration: BoxDecoration(
                gradient: isAccepting
                    ...
                    : const LinearGradient(
                        colors: [Color(0xFF1B7F6E), Color(0xFF2ED9B8)],
        ...
        child: Center(
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Container(
                ...
                height: 100,
                decoration:
                    BoxDecoration(shape: BoxShape.circle, color: _primary.withOpacity(0.08)),
