...
class _UploadDocumentsScreenState extends State<UploadDocumentsScreen>
  ...
  @override
  void initState() { super.initState(); _checkExistingDocuments(); }
    ...
    try {
      for (final item in widget.cartItems) {
        ...
        if (res.statusCode == 200 && data["already_uploaded"] == true) {
          if (!mounted) return;
          setState(() { _alreadyUploaded = true; _uploadedDocs = data["documents"] ?? []; });
          return;
        }
      }
    } catch (e) { debugPrint("CHECK DOC ERROR: $e"); }
  }
  ...
  Future<void> _pickDocuments() async {
    ...
    if (result != null) setState(() => _selectedFiles = result.paths.map((e) => File(e!)).toList());
  }

  Future<void> _uploadDocuments() async {
    if (_selectedFiles.isEmpty) { _toast("Please select at least one document", _ToastType.error); return; }
    ...
    try {
      for (final item in widget.cartItems) {
        ...
        if (res.statusCode != 200 || data["success"] != true) throw Exception(data["message"] ?? "Upload failed");
  ...
  void _goToPayment() => Navigator.pushReplacement(context, MaterialPageRoute(
    builder: (_) => PaymentScreen(
      userId: widget.userId, cartItems: widget.cartItems,
      subtotal: widget.subtotal, serviceCharge: widget.serviceCharge,
      discount: widget.discount, couponCode: widget.couponCode,
      slot: widget.slot, date: widget.date, location: widget.location,
  ...
  void _toast(String msg, _ToastType type) {
    ...
    ScaffoldMessenger.of(context)
      ...
      ..showSnackBar(SnackBar(
        ...
        // Push it to the top via a large top margin hack using SnackBarBehavior + overlay
    ...
    child: SafeArea(bottom: false, child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          ...
          const SizedBox(width: 10),
          // Title (single line, tight)
          const Expanded(child: Text("Upload Medical Docs",
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: ready ? color.withOpacity(0.7) : AppColors.primary.withOpacity(0.25), width: 1.8),
        ...
        child: Column(children: [
          Stack(alignment: Alignment.center, children: [
            ...
            Icon(
              _alreadyUploaded ? Icons.check_circle_rounded
                  : _selectedFiles.isEmpty ? Icons.upload_file_rounded : Icons.add_circle_rounded,
              size: 32, color: color),
          ]),
          const SizedBox(height: 12),
          Text(
            _alreadyUploaded ? "Documents Already Uploaded"
          ...
          if (!_alreadyUploaded && _selectedFiles.isNotEmpty) ...[
            ...
            Container(
              ...
              child: Text("${_selectedFiles.length} file${_selectedFiles.length > 1 ? 's' : ''} selected",
  ...
  Widget _bottomCTA() {
    ...
    final ctaIcon = _alreadyUploaded ? Icons.check_circle_rounded
        : _selectedFiles.isEmpty ? Icons.folder_open_rounded : Icons.lock_rounded;
    final ctaLabel = _alreadyUploaded
        ? "Documents Uploaded — Continue"
        : _selectedFiles.isEmpty ? "Select Documents to Continue" : "Upload & Continue to Payment";
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text("Total Amount", style: TextStyle(color: Colors.grey.shade500, fontSize: 13, fontWeight: FontWeight.w500)),
          ...
          child: ElevatedButton(
            ...
            child: _loading
                ...
                : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    const SizedBox(width: 8),
                    Text(ctaLabel, style: const TextStyle(fontSize: 14.5, fontWeight: FontWeight.bold, color: Colors.white)),
                  ]),
          ),
        ),
