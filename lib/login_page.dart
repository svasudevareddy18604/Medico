...
class _LoginPageState extends State<LoginPage> {
  ...
  Future loginUser() async {
    if (!acceptTerms) return showToast("Please accept Terms & Conditions", type: ToastType.warning);
    ...
    try {
      ...
      final response = await http.post(Uri.parse("${Api.baseUrl}/login"),
          ...
          body: jsonEncode({"email": emailController.text.trim(), "password": passwordController.text.trim(), "fcm_token": fcmToken}));
      setState(() => loading = false);
      final data = jsonDecode(response.body);
      if (response.statusCode != 200) return showToast(data["message"] ?? "Login failed", type: ToastType.error);
      ...
      if (!mounted) return;
      if (role == "admin") {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => AdminHomePage(userId: userId)));
      } else if (role == "care_seeker") {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => CareSeekerHome(userId: userId)));
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // ── HEADER — same as before, only MEDICO text is new ──
          Container(
            ...
            child: Column(children: [
              ...
              const Text("CARE ANYTIME ANYWHERE",
        ...
        return ShaderMask(
          shaderCallback: (bounds) => LinearGradient(
            ...
            colors: const [
              Colors.white,
              Colors.white,
              Color(0xFFB2FFEE), // soft teal-white shimmer peak
              Colors.white,
              Colors.white,
            ],
            stops: [
